...
Keep REAPER instances identifiable and clean up disposable instances after each bounded check. Record the profile and process identity when launching one. Before closing an agent-owned instance, snapshot dirty disposable tabs to new files, verify the original project files were not overwritten, stop its bridge, and close only that exact process. Leave shared or producer instances and their dirty tabs open; ask John when ownership is unclear.
